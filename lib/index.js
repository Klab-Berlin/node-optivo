...
optivo.prototype.subscribe = function(data, callback){
	...
	delete sendData.parameter.authCode;
...
optivo.prototype.uploadpersonalizedattachments = function(data, callback){
	...
	for (var i in data.files) {
		...
		reqData.postData = reqData.postData.concat([
			'--' + boundary + '\r\n',
			'Content-Disposition: form-data; name="bmFile"; filename="' + data.files[i].filename+'.pdf"\r\n',
...
function request(data, callback){
	...
	var parameterArray = ["bmEncoding=utf-8"];
	var pathArray = ['','http', 'form', data.authCode, data.command];
